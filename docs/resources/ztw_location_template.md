...
layout: "zscaler"
page_title: "ZTW: location_template"
description: |-
...
Use the **ztw_location_template**  resource allows the creation and management of location templates available in the Zscaler Cloud and Branch Connector Portal. This resource can then be associated with ZTW locations and provisioning URLs.
...
**ztw_location_template** can be imported by using `<TEMPLATE_ID>` or `<TEMPLATE_NAME>` as the import ID.

For example:

```shell
terraform import ztw_location_template.example <rule_id>
```

or

```shell
terraform import ztw_location_template.example <rule_name>
```
