...
description: |-
  ...
  API documentation https://automate.zscaler.com/docs/api-reference-and-guides/api-reference/zcloudconnector/policy-management/ec-rule-z-resource-create-rdr-rule
  Creates and manages Traffic Forwarding Rules.
---

# ztc_traffic_forwarding_rule (Resource)
...
Use the **ztc_traffic_forwarding_rule** resource allows the creation and management of traffic forwarding in the Zscaler Cloud and Branch Connector Portal.
...
**ztc_traffic_forwarding_rule** can be imported by using `<RULE ID>` or `<RULE NAME>` as the import ID.

For example:

```shell
terraform import ztc_traffic_forwarding_rule.example <rule_id>
```

or

```shell
terraform import ztc_traffic_forwarding_rule.example <rule_name>
```
