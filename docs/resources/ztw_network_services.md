...
layout: "zscaler"
page_title: "ZTW: network_services"
description: |-
...
Use the **ztw_network_services** resource allows the creation and management of network services available in the Zscaler Cloud and Branch Connector Portal. This resource can then be associated with ZTW traffic forwarding rule.

## Example Usage - Create Network Services

```hcl
resource "ztw_network_services" "example" {
  name        = "example"
  description = "example"
  src_tcp_ports {
    start = 5000
...
**ztw_network_services** can be imported by using `<SERVICE_ID>` or `<SERVICE_NAME>` as the import ID.

For example:

```shell
terraform import ztw_network_services.example <rule_id>
```

or

```shell
terraform import ztw_network_services.example <rule_name>
```
