...
```hcl
resource "ztc_ip_pool_groups" "example" {
...
**ztc_ip_pool_groups** can be imported by using `<GROUP_ID>` or `<GROUP_NAME>` as the import ID.

For example:

```shell
terraform import ztc_ip_pool_groups.example <rule_id>
```

or

```shell
terraform import ztc_ip_pool_groups.example <rule_name>
```