...
layout: "zscaler"
page_title: "ZTW: provisioning_url"
description: |-
...
Use the **ztw_provisioning_url** resource allows the creation and management of provisioning URLs available in the Zscaler Cloud and Branch Connector Portal.
...
**ztw_provisioning_url** can be imported by using `<PROV_URL_ID>` or `<PROV_URL_NAME>` as the import ID.

For example:

```shell
terraform import ztw_provisioning_url.example <rule_id>
```

or

```shell
terraform import ztw_provisioning_url.example <rule_name>
```
