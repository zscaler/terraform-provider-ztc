...
layout: "zscaler"
page_title: "ZTW: traffic_forwarding_dns_rule"
description: |-
...
# ztw_traffic_forwarding_dns_rule (Resource)

* [Official documentation](https://help.zscaler.com/cloud-branch-connector/about-dns-policies)
* [API documentation](https://help.zscaler.com/cloud-branch-connector/about-dns-policies)

Use the **ztw_traffic_forwarding_dns_rule** resource allows the creation and management of traffic forwarding DNS rule in the Zscaler Cloud and Branch Connector Portal.
...
resource "ztw_traffic_forwarding_dns_rule" "this" {
  name              = "DNS_Rule01"
  ...
  dest_ip_groups {
    id = [ztw_ip_destination_groups.dstn_fqdn.id]
  }
  locations {
    id = [data.ztw_location_management.this.id]
  }
  dns_gateway {
    id   = ztw_dns_forwarding_gateway.this.id
    name = ztw_dns_forwarding_gateway.this.name
  }
}
```
...
resource "ztw_ip_pool_groups" "this" {
...
resource "ztw_traffic_forwarding_dns_rule" "this" {
  ...
  zpa_ip_group {
    id   = ztw_ip_pool_groups.this.id
    name = ztw_ip_pool_groups.this.name
  }
}
```
...
**ztw_traffic_forwarding_dns_rule** can be imported by using `<RULE ID>` or `<RULE NAME>` as the import ID.

For example:

```shell
terraform import ztw_traffic_forwarding_dns_rule.example <rule_id>
```

or

```shell
terraform import ztw_traffic_forwarding_dns_rule.example <rule_name>
```