---
layout: "zscaler"
page_title: "Provider: Zero Trust Workload (ZTW)"
description: |-
    The Zero Trust Workload  provider is used to interact with Zero Trust Workload (ZTW) API
---

# Zero Trust Workload  (ZTW) Provider

The Zero Trust Workload  provider is used to interact with ZTW API, to automate the provisioning of new locations, IPSec and GRE tunnels, URL filtering policies, Cloud Firewall Policies, DLP Dictionaries, Local Accounts etc. The provider is intended to save time and reducing configuration errors. With this ZTW provider, DevOps teams can automate their security and transform it into DevSecOps workflows. To use this  provider, you must create ZTW API credentials.
...
The ZTW Terraform Provider now offers support for [OneAPI](https://help.zscaler.com/oneapi/understanding-oneapi) Oauth2 authentication through [Zidentity](https://help.zscaler.com/zidentity/what-zidentity).
...
```hcl
# Configure the Zero Trust Workload Provider
terraform {
    required_providers {
        ztw = {
            version = "~> 1.0.0"
...
# Configure the ZTW Provider (OneAPI Authentication)
#
...
# Configure the ZTW Provider (OneAPI Authentication) - Private Key
#
# NOTE: Change place holder values denoted by brackets to real values, including
# the brackets.
#
...
For the resources and data sources examples, please check the [examples](https://github.com/zscaler/terraform-provider-ztw/tree/master/examples) directory.
...
applicable to APIs within the various products. For example, in ZTW, navigate to `Administration -> Role
...
select API Resources. Click the `View` icon to the right of Zscaler APIs and under the ZTW
...
# Configure the ZTW Provider (Legacy Authentication)
#
# NOTE: Change place holder values denoted by brackets to real values, including
# the brackets.
#
...
# block.
provider "ztw" {
  username            = "[ZTW_USERNAME]"
  password            = "[ZTW_PASSWORD]"
  api_key             = "[ZTW_API_KEY]"
  ztw_cloud           = "[ZTW_CLOUD]"
...
You can provide credentials via the `ZTW_USERNAME`, `ZTW_PASSWORD`, `ZTW_API_KEY`, `ZTW_CLOUD`, `ZSCALER_USE_LEGACY_CLIENT` environment variables, representing your ZTW `username`, `password`, `api_key`,  `ztw_cloud` and `use_legacy_client` respectively.
...
| `ztw_cloud`       | _(String)_ The host and basePath for the cloud services API is `$zsapi.<Zscaler Cloud Name>/api/v1`.| `ZTW_CLOUD` |
| `use_legacy_client`       | _(Bool)_ Enable use of the legacy ZTW API Client.| `ZSCALER_USE_LEGACY_CLIENT` |

```sh
# Change place holder values denoted by brackets to real values, including the
# brackets.

$ export ZTW_USERNAME="[ZTW_USERNAME]"
$ export ZTW_PASSWORD="[ZTW_PASSWORD]"
$ export ZTW_API_KEY="[ZTW_API_KEY]"
$ export ZTW_CLOUD="[ZTW_CLOUD]"
...
$env:ztw_cloud = '<zscaler_cloud_name>'
...
### ZTW Configuration Activation

The ZTW platform requires every configuration to be activated. To make this process more flexible, made available a dedicated out of band CLI method described here [ztw activator](guides/ztw-activator-overview.md) or leverage the dedicated activation resource `ztw_activation_status`.
...
* `username` - (Optional) Administrator account used when authenticating to the legacy Zscaler API framework. Can also be sourced from the `ZTW_USERNAME` environment variable.

* `password` - (Optional) Administrator password used when authenticating to the legacy Zscaler API framework. Can also be sourced from the `ZTW_PASSWORD` environment variable.

* `api_key` - (Optional) API key found in the Zero Trust Workload  portal `Administration > API Key Mangement > API Key Management`. Can also be sourced from the `ZTW_API_KEY` environment variable. Ensure you have the following SKU enabled `Z_API`

* `ztw_cloud` - (Optional) This refers to the Zscaler cloud name where api calls will be forward to. Can also be sourced from the `ZTW_CLOUD` environment variable.
