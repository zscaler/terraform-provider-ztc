...
```bash
$ make build13 && sudo make ztwActivator
```

~> You may or may not have to use `sudo` in order to successfully install the `ztwActivator` cli in the path.

Finally, you can invoke this binary after `terraform apply` or `terraform destroy` or invoke all cli commands together.

```bash
$ terraform init && terraform apply && ztwActivator
```
