...
func init() {
	testAccProvider = ZTWProvider()
	testAccProviders = map[string]*schema.Provider{
		"ztw": testAccProvider,
	}

	testAccProviderFactories = map[string]func() (*schema.Provider, error){
		"ztw": func() (*schema.Provider, error) {
...
func TestMain(m *testing.M) {
	...
	// going over the wire
	if os.Getenv("ZTW_VCR_TF_ACC") != "play" {
...
func TestProvider(t *testing.T) {
	if err := ZTWProvider().InternalValidate(); err != nil {
		t.Fatalf("err: %s", err)
	}
}
