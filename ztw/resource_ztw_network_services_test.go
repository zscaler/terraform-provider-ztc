...
func TestAccResourceNetworkServices_Basic(t *testing.T) {
	var services networkservices.NetworkServices
	resourceTypeAndName, _, generatedName := method.GenerateRandomSourcesTypeAndName(resourcetype.NetworkServices)
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckFWNetworkServicesConfigure(resourceTypeAndName, initialName, variable.NetworkServicesDescription, variable.NetworkServicesType),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWNetworkServicesExists(resourceTypeAndName, &services),
					resource.TestCheckResourceAttr(resourceTypeAndName, "name", initialName),
					resource.TestCheckResourceAttr(resourceTypeAndName, "description", variable.NetworkServicesDescription),
					resource.TestCheckResourceAttr(resourceTypeAndName, "type", variable.NetworkServicesType),
			...
			// Update test
			{
				Config: testAccCheckFWNetworkServicesConfigure(resourceTypeAndName, updatedName, variable.NetworkServicesDescription, variable.NetworkServicesType),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckFWNetworkServicesExists(resourceTypeAndName, &services),
...
func testAccCheckFWNetworkServicesDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != resourcetype.NetworkServices {
			continue
		}
